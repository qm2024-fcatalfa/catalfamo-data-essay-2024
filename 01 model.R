...
model2 <- glm(citation ~ female + minority + female * minority +
                constit + tenure + authorities + elite + termmeritsper, 
              family = "poisson", data = data)
summary(model2)
...
model4 <- glm.nb(citation ~ female * minority + constit + tenure + authorities + elite + termmeritsper, 
                 data = data,
                 control = glm.control(maxit = 100))
summary(model4)
